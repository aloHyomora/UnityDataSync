...
def start_server():
    ...
    # 소켓 객체 생성
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as so: # 네트워크 리소스 사용 후 자동으로 정리, 소켓 사용이 끝나면 자동으로 소켓을 닫음.
        ...
        with conn:
            ...
            while True:
                try:
                    ...
                    print("Received int : ", receivedNumber, "-> Conver to float : ", float_number)
                
                    modified_number = float_number * 2
                    print("Send float value * 2 = ", modified_number)
                
                    float_data = str(modified_number).encode("utf-8") + b'\n' # 실수를 문자열로 변환 후 바이트로 인코딩하여 전송
